...
public class StudentsController {
    ...
    {
        var student = studentService.findById(id).orElseThrow(() -> new HandsonException("Student with ID " + id + " not found"));
        return new ResponseEntity<>(StudentOut.of(student, awsService)  , HttpStatus.OK);
    }
...
//    @RequestMapping(value = "/{id}/image", method = RequestMethod.PUT)
    ...
    public ResponseEntity<?> uploadStudentImage(@PathVariable Long id,  @RequestParam("image") MultipartFile image)
    {
        Optional<Student> dbStudent = studentService.findById(id);
        if (dbStudent.isEmpty()) throw new HandsonException("Student with id: " + id + " not found");
        String bucketPath = "apps/nirsh/student-" +  id + ".png" ;
